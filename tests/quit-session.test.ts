...
const extensionPath = path.resolve(
	import.meta.dir,
	"../omp/extensions/quit-session.ts",
);
const supported =
	process.platform === "linux" &&
	Boolean(Bun.which("python3")) &&
	Boolean(Bun.which("bash"));
...
async function createRunner(): Promise<{ directory: string; runner: string }> {
	const directory = await fs.mkdtemp(
		path.join(os.tmpdir(), "quit-session-test-"),
	);
...
test.skipIf(!supported)(
	"kill_terminal closes an owned parent shell after the child exits",
	async () => {
...
test.skipIf(!supported)(
	"kill_terminal never signals an owned non-shell supervisor",
	async () => {
		const { directory, runner } = await createRunner();
...
`,
		...
		const supervisor = Bun.spawn([process.execPath, supervisorPath, runner], {
			stdout: "pipe",
			stderr: "pipe",
		});
