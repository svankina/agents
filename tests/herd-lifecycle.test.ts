...
test("quit evidence waits for idle and excludes continuations, new sessions and cleanup", async () => {
		...
		handlers.get(name)!(
			{
				...
				toolCallId: callId,
				messages: [
					{ role: "toolResult", toolCallId: callId, toolName: "write" },
				],
				...event,
			},
			ctx,
	...
	try {
		...
		await emit("tool_result", {
			...
			isError: false,
			details: {
				xdev: { tool: "quit_session", mode: "execute", args: {}, tier: "exec" },
			},
		});
		...
		for (let i = 1; i < observations.length; i++)
			expect(observations[i].seq).toBeGreaterThan(observations[i - 1].seq);
	} finally {
