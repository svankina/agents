...
These instructions apply to Claude Code only. Forked from
`~/src/agents/shared/AGENTS.md` on 2026-08-05 — edits to the shared file no
longer reach Claude Code; edit this file instead. Codex, Pi, and omp still use
the shared core.

## How this file is maintained

`~/.claude/CLAUDE.md` is GENERATED by `agents-sync` from
`~/src/agents/claude/AGENTS.md` (this file) + `~/src/agents/shared/USER.md` +
`~/src/agents/claude/local.md`. To change Claude's standing instructions:
edit this file, then run `~/src/agents/bin/agents-sync sync`. Never edit the
generated file directly — `agents-sync check` flags drift and `sync` will
refuse to clobber it silently.

Durable facts about the user — preferences, environment, systems — go in
`~/src/agents/shared/USER.md`, which is still shared by every agent and is
concatenated in between these instructions and the agent-specific delta. Keep
instructions here and facts there.
