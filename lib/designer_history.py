...
_IMAGE_REF = re.compile(r"(?:file:///|~/|/)[^\s<>\"`]*?\.(?:png|jpe?g|webp|gif|svg)(?=$|[\s<>\"`),;:!?\]])", re.I)
...
class History:
    ...
    def _event(self, eid, peer, direction, body, timestamp, source, sender, to, reply=None,
        ...
        for match in _IMAGE_REF.finditer(body):
            # Never extract the path portion of an HTTP or other remote URL.
            prefix = body[:match.start()].rsplit(None, 1)[-1] if body[:match.start()].strip() else ''
            if '://' in prefix or (match.start() and body[match.start()-1] in '/:'):
                continue
            ...
            self._reference(cid, str(Path(raw).expanduser()), eid, timestamp)
        return eid
    ...
    def sync(self, peers: list[dict]) -> None:
        ...
        with self.db:
            ...
            for peer in peers:
                self._peer(peer, live=True)
            # Import all references first, then observe each current path only once. This cannot
            # recover historical bytes from an overwritten path, and never pretends otherwise.
            for ref in self.db.execute('SELECT * FROM refs').fetchall():
                try:
                    path = Path(ref['path'])
                    info = path.stat(follow_symlinks=False)
                    signature = (info.st_dev, info.st_ino, info.st_size, info.st_mtime_ns, info.st_ctime_ns)
                    key = (ref['contact'], ref['path'])
                    if self._observations.get(key) != signature:
                        self._revision(ref['contact'], ref['path'], path, '', ref['event'])
                    observations[key] = signature
                except (OSError, ValueError) as error:
                    self._warn(f'Image unavailable at observation; historical bytes not recovered: {ref["path"]}: {error}')
    ...
    def snapshot(self) -> dict:
        ...
        return dict(sampledAt=float(sampled[0]) if sampled else None,
                    ...
                    warnings=[row[0] for row in self.db.execute('SELECT message FROM warnings ORDER BY message')],
